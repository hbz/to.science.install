...
cat $ARCHIVE_HOME/logs/titleObjects.txt | parallel --jobs 5 ./updatePid.sh {} http://localhost:9000 $yesterday  >$log 2>&1
cp $log /tmp/updateMetadata
echo "Summary" >> $log
echo "Updated
grep  "Enrichment.*succeeded!" /tmp/updateMetadata | grep -v "Not updated"|grep -o "$INDEXNAME:[^\ ]*" >> $log
cd -
